...
class App:
    ...
    def setup_ui(self):
        ...
        ttk.Label(self.ventana, text="Pecio").grid(row=3,column=0)
        self.precio=tk.StringVar()
        ttk.Entry(self.ventana, textvariable=self.precio).grid(row=3,column=1)
  
        ttk.Label(self.ventana, text="Almacenamiento").grid(row=4,column=0)
        self.almacenamiento=tk.StringVar()
        ttk.Entry(self.ventana, textvariable=self.almacenamiento).grid(row=4,column=1)

        ttk.Label(self.ventana, text="Duración de Batería").grid(row=5,column=0)
        self.duracionBateria=tk.StringVar()
        ttk.Entry(self.ventana, textvariable=self.duracionBateria).grid(row=5,column=1)
        
        ttk.Button(self.ventana,text="Agregar lapto",command=self.agregarLapto).grid(row=6,column=0)
    
        self.mostrarLaptos=tk.Text(self.ventana,height=10,width=50)
        self.mostrarLaptos.grid(row=7,column=0,columnspan=2)

        self.canva=tk.Canvas(self.ventana, width=200,height=200)
        self.canva.grid(row=1,column=3,rowspan=6)

    def agregarLapto(self):
        nuevaLapto=LaptopBusiness(self.marca.get(),self.procesador.get(),self.memoria.get(),self.precio.get(),self.almacenamiento.get(),self.duracionBateria.get())
